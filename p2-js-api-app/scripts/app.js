...
const populateModalContent = (modalElements, recipe) => {
  ...
  exIngredientsElement.innerHTML = formatIngredients(recipe.extendedIngredients || [])

  instructionTitleElement.innerHTML  = "Instructions";
  instructionElement.innerHTML = formatInstructions(recipe.analyzedInstructions || []);

  nutritionTitleElement.innerHTML = "Nutrition";
  nutritionElement.innerHTML = formatNutritions(recipe.nutrition?.nutrients || [])
}

const deleteRecipe = (id, modal) => {
  localStorage.removeItem(`${id}`);

  const recipeCard = document.querySelector(`#recipeCard img[id="${id}"]`).parentElement;
